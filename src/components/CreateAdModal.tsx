  ...
  async function handleCreateAd(event: FormEvent){
    ...
    return(
        <Dialog.Portal>
          ...
          <Dialog.Content className='fixed bg-[#2A2634] py-6 px-6 sm:py-8 sm:px-10 text-white top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-lg w-[320px] sm:w-[480px] shadow-lg shadow-black/25'>
            <Dialog.Title className='text-3xl font-black'>Publique um anúncio</Dialog.Title>

              <form onSubmit={handleCreateAd} className='mt-8 flex flex-col gap-4'>
                <div className='flex flex-col gap-2'>
                  <label htmlFor="game" className='font-semibold' >Qual o game?</label>
                  <select defaultValue="" id='game' name='game' className='bg-zinc-900 py-3 px-4 rounded text-sm placeholder:text-zinc-500 appearance-none'>
                    <option disabled value="">Selecione o game que deseja jogar</option>
                    {games.map(game => { return <option value={game.id} key={game.id}>{game.title}</option>})}
                  </select>
                </div>
                ...
                <div className='grid grid-cols-2 gap-6'>
                  <div className='flex flex-col gap-2'>
                    <label htmlFor="yearsPlaying" className='text-[15px] sm:text-1xl'>Joga a quantos anos?</label>
                    <Input name='yearsPlaying' id='yearsPlaying' type="number" placeholder='Tudo bem ser ZERO' />
                  </div>
                  <div className='flex flex-col gap-2'>
                    <label htmlFor="discord" className='text-[15px] sm:text-1xl'>Qual seu Discord?</label>
                    <Input name='discord' id='discord' type="text" placeholder='Usuario#0000' />
                  </div>
                </div>
                <div className='flex gap-6'>
                  <div className='flex flex-col gap-2'>
                    <label htmlFor="weekDays" className='text-[15px] sm:text-1xl'>Quando costuma jogar?</label>
                      <ToggleGroup.Root 
                      type='multiple' 
                      className='grid grid-cols-3 sm:grid-cols-4 gap-[5px] sm:gap-2'
                      value={weekDays}
                  ...
                  <div className='flex flex-col gap-2 sm:flex-1'>
                    <label htmlFor="hoursStart" className='text-[15px] sm:text-1xl'>Qual horário do dia?</label>
                ...
                <label className='nt-2  items-center flex gap-2 text-sm'>
                  <Checkbox.Root 
                ...
                <footer className='nt-4 flex justify-center sm:justify-end gap-4'>
